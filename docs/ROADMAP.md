...
| **2 — Unity vertical slice** | SimBridge runs sim in-process; Input System (KB/M + pad); 2.5D PlayerView + 1 arena; HUD; menu→lobby→play→results loop; starter CC0 assets + SFX; settings | Plays solo vs bots in Unity |
| **3 — Breadth** | Remaining 13 minigames (TDD) + view modules + 6 arenas + 16 players/accessories + progression/unlocks + local leaderboard | `dotnet test`; play in Unity |
